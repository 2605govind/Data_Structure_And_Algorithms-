// next_largest_element_aka_nearest_greater_to_right
#include <iostream>
#include <vector>
#include <stack>
#include <algorithm>
...
class Solution_1 {
public:
    std::vector<int> nearestGreaterToRight(std::vector<int> &arr) {
        int n = arr.size();
...
int main() {
    ...
    while(testCase--) {
        ...
        // std::vector<int> ans = s1.nearestGreaterToRight(nums); // TC O(N^2)
        std::vector<int> ans = s2.nearestGreaterToRight(nums); // TC O(N)
